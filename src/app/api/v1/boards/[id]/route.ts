import { NextResponse } from "next/server";

import { errorResponse, successResponse, toBoardResponse } from "@/lib/api-response";
import { auth } from "@/lib/auth/config";
import { deleteBoard, findBoardSummaryById, updateBoard } from "@/lib/db/boards";
import { AppError } from "@/lib/errors";
import { logger } from "@/lib/logger";
import { fieldErrorsFromZod } from "@/lib/validations/helpers";
import { updateBoardSchema } from "@/lib/validations/board";

export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  const session = await auth();
  const userId = session?.user?.id;

  if (!userId) {
    return NextResponse.json(
      errorResponse(new AppError("UNAUTHORIZED", "Authentication required", 401)),
      { status: 401 },
    );
  }

  try {
    const { id } = await context.params;
    const board = await findBoardSummaryById(id);

    if (!board || board.userId !== userId) {
      return NextResponse.json(
        errorResponse(new AppError("NOT_FOUND", "Board not found", 404)),
        { status: 404 },
      );
    }

    return NextResponse.json(successResponse(toBoardResponse(board)));
  } catch (error) {
    logger.error("boards.get.unexpected_error", {
      error: error instanceof Error ? error.message : String(error),
      userId,
    });
    throw error;
  }
}

export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  const session = await auth();
  const userId = session?.user?.id;

  if (!userId) {
    return NextResponse.json(
      errorResponse(new AppError("UNAUTHORIZED", "Authentication required", 401)),
      { status: 401 },
    );
  }

  try {
    const { id } = await context.params;
    const json = await request.json();
    const parsed = updateBoardSchema.safeParse(json);

    if (!parsed.success) {
      return NextResponse.json(
        errorResponse(new AppError("VALIDATION_ERROR", "Invalid board update input", 400), {
          fields: fieldErrorsFromZod(parsed.error),
        }),
        { status: 400 },
      );
    }

    const board = await updateBoard(id, userId, parsed.data);

    if (!board) {
      return NextResponse.json(
        errorResponse(new AppError("NOT_FOUND", "Board not found", 404)),
        { status: 404 },
      );
    }

    return NextResponse.json(successResponse(toBoardResponse(board)));
  } catch (error) {
    if (error instanceof SyntaxError) {
      return NextResponse.json(
        errorResponse(new AppError("BAD_REQUEST", "Request body must be valid JSON", 400)),
        { status: 400 },
      );
    }

    logger.error("boards.update.unexpected_error", {
      error: error instanceof Error ? error.message : String(error),
      userId,
    });
    throw error;
  }
}

export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  const session = await auth();
  const userId = session?.user?.id;

  if (!userId) {
    return NextResponse.json(
      errorResponse(new AppError("UNAUTHORIZED", "Authentication required", 401)),
      { status: 401 },
    );
  }

  try {
    const { id } = await context.params;
    const deleted = await deleteBoard(id, userId);

    if (!deleted) {
      return NextResponse.json(
        errorResponse(new AppError("NOT_FOUND", "Board not found", 404)),
        { status: 404 },
      );
    }

    return new NextResponse(null, { status: 204 });
  } catch (error) {
    if (error instanceof Error && error.name === "PrismaClientValidationError") {
      return NextResponse.json(
        errorResponse(new AppError("NOT_FOUND", "Board not found", 404)),
        { status: 404 },
      );
    }

    logger.error("boards.delete.unexpected_error", {
      error: error instanceof Error ? error.message : String(error),
      userId,
    });
    throw error;
  }
}
